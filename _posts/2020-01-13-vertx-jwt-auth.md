...
In this simple example we have a small authorization server, where we can create a users account, log in and then access a protected route.

Remember that, to access a protected route, we need to add the following option `Authorization: Bearer <token>` in the request headers.
