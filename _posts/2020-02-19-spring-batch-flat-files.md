---
layout: post
title: Java - Spring Batch Flat File
date: 2020-02-19 20:18:00 +0100
category: Dev
tags: spring-batch java
---

This is reference of how to use Spring Batch to read a simple text file, proccess it and save its data into MySQL database.

#### Example

It's common to see old systems using text files to integrate their information into another system. This tabular file is an example that represent
this kind of files which in a real application it can be really large containing thousands of lines. 
...
This is the required dependencies to set up Spring Batch.
...
This class is a listener which is executed after job completion. It can be used for clean up, notification, etc.
...
This class configures the job as well as the steps that should be done in order to migrate all data from text files into database.
...
This is a custom implementation of `SimpleRecordSeparatorPolicy` sometimes we want to skip reading header or footer data from a text file or even
custom lines.
...
Custom implementation of `Spring RowMapper` to map database result rows to Java objects.
...
This class is responsible to map file data into Java objects attributes.
...
Here is the bean that contains the logic to read your source data.
...
Once Spring Batch reads the file data and create its chunks, it sends the data to the `ItemProcessor` where all logic to manipulate data should be done.
...
Finally, Spring Batch uses this class to execute the logic to write the processed data wherever you want.
